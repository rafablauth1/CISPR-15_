...
import { boardPadrao, STATUS, type BoardCheck, type StatusTarefa } from '@/lib/check/tipos'
...
export async function POST(req: NextRequest) {
  try {
    const { id, title, status, note, level } = (await req.json()) as {
      id?: string; title?: string; status?: StatusTarefa; note?: string; level?: 'info' | 'error'
    }
    if (!id && !title) {
